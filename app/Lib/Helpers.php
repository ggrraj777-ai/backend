...
if (!function_exists('getFareSetting')) {
    function getFareSetting($key, $default = 0)
    {
        $config = businessConfig($key, 'fare_calculation');
        if (!$config) return $default;
        $val = $config->value;
        if (is_array($val)) {
            if (array_key_exists('value', $val)) return $val['value'];
            $first = reset($val);
            return is_numeric($first) ? (float)$first : ($first ?? $default);
        }
        return is_numeric($val) ? (float)$val : ($val ?: $default);
    }
}

if (!function_exists('calculateFareBreakdown')) {
    function calculateFareBreakdown($fare)
    {
        $fare = (float)$fare;
        $gstOnRidePercent = (float)getFareSetting('gst_on_ride_percent', 5);
        $platformFeeType = getFareSetting('platform_fee_type', 'flat');
        $platformFeeValue = (float)getFareSetting('platform_fee_value', 0);
        $gstOnFeePercent = (float)getFareSetting('gst_on_fee_percent', 18);
        $cashbackPercent = (float)getFareSetting('cashback_percent', 0);

        $gstOnRide = round($fare * $gstOnRidePercent / 100, 2);
        $platformFeeBase = $platformFeeType === 'percent' ? round($fare * $platformFeeValue / 100, 2) : round($platformFeeValue, 2);
        $platformFeeGst = round($platformFeeBase * $gstOnFeePercent / 100, 2);
        $platformFeeTotal = round($platformFeeBase + $platformFeeGst, 2);
        $cashback = round($fare * $cashbackPercent / 100, 2);
        $customerPays = round($fare + $gstOnRide + $platformFeeTotal, 2);
        $driverReceives = round($fare, 2);
        $gauvaBalance = round($customerPays - ($driverReceives + $cashback + $gstOnRide + $platformFeeGst), 2);

        return [
            'fare' => $fare,
            'gst_on_ride' => $gstOnRide,
            'platform_fee_base' => $platformFeeBase,
            'platform_fee_gst' => $platformFeeGst,
            'cashback' => $cashback,
            'customer_pays' => $customerPays,
            'driver_receives' => $driverReceives,
            'gauva_balance' => $gauvaBalance,
            'settings' => [
                'gst_on_ride_percent' => $gstOnRidePercent,
                'platform_fee_type' => $platformFeeType,
                'platform_fee_value' => $platformFeeValue,
                'gst_on_fee_percent' => $gstOnFeePercent,
...
if (!function_exists('getRoutes')) {
    function getRoutes(array $originCoordinates, array $destinationCoordinates, array $intermediateCoordinates = [], array $drivingMode = ["DRIVE"])
    {
        ...
        if ($response->successful()) {
            ...
            preg_match('/(\d+)S/', $durationInTraffic, $trafficMatches);
...
if (!function_exists('checkReverbConnection')) {
    ...
    {
        try {
            $socketUrl = env('REVERB_HOST');
            $socketScheme = env('REVERB_SCHEME');
            $socketPort = env('REVERB_PORT');
            Http::get($socketScheme . '://' . $socketUrl . ':' . $socketPort);
            return true;
        } catch (\Exception $e) {
            return false;
        }
...
if (!function_exists('textVariableDataFormat')) {
                                    ...
                                    $referralRewardAmount = null, $otp = null, $parcelId = null, $approximateAmount = null)
    {
        ...
        if ($value) {
            ...
            if ($approximateAmount) {
                $data = str_replace("{approximateAmount}", $approximateAmount, $data);
            }
        }
